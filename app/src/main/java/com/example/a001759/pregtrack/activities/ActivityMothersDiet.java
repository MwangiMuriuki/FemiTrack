...
import com.example.a001759.pregtrack.databinding.ActivityMothersDietBinding;
...
public class ActivityMothersDiet extends AppCompatActivity {

    FirebaseFirestore firebaseFirestore;
    AdapterOtherArticles adapterOtherArticles;
    List<ModelClassOtherArticles> list;

    ActivityMothersDietBinding binding;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        binding = DataBindingUtil.setContentView(this, R.layout.activity_mothers_diet);
        binding.mothersDietToolBar.setNavigationIcon(R.drawable.ic_back);
        setSupportActionBar(binding.mothersDietToolBar);
        Objects.requireNonNull(getSupportActionBar()).setTitle("Mother`s Diet");
        ...
        adapterOtherArticles = new AdapterOtherArticles(list, firebaseFirestore, ActivityMothersDiet.this);

        binding.recyclerView.setAdapter(adapterOtherArticles);

        firebaseFirestore.collection("mothers_diet_articles").get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            @Override
