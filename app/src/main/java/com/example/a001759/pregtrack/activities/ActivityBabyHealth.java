...
import com.example.a001759.pregtrack.databinding.ActivityBabyHealthBinding;
...
public class ActivityBabyHealth extends AppCompatActivity {

    FirebaseFirestore firebaseFirestore;
    AdapterOtherArticles adapterOtherArticles;
    List<ModelClassOtherArticles> list;

    ActivityBabyHealthBinding binding;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        binding = DataBindingUtil.setContentView(this, R.layout.activity_baby_health);
        binding.babyHealthToolBar.setNavigationIcon(R.drawable.ic_back);
        setSupportActionBar(binding.babyHealthToolBar);
        Objects.requireNonNull(getSupportActionBar()).setTitle("Baby`s Health");
        ...
        adapterOtherArticles = new AdapterOtherArticles(list, firebaseFirestore, ActivityBabyHealth.this);
        binding.recyclerView.setAdapter(adapterOtherArticles);

        firebaseFirestore.collection("babys_health_articles").get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            @Override
